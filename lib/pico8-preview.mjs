import {atlasRgbaToPicoIndices,buildPico8Cart} from './pico8-cart.mjs';

const RUNTIME_BASE=new URL('../preview-runtime/',import.meta.url);
const ATLAS_URL=new URL('../assets/pico8-atlas.png',import.meta.url);
...
function loadClassicScript(url){
  return new Promise((resolve,reject)=>{
    ...
    s.src=url.href;s.async=true;s.dataset.celesteRuntime=url.href;
    s.onload=resolve;s.onerror=()=>reject(new Error(`Failed to load ${url.pathname}`));
...
async function getFake08(){
  if(!fake08Promise)fake08Promise=(async()=>{
    ...
    const Module=await globalThis.Fake08Module({
      ...
      print:msg=>console.debug('[Fake-08]',msg),
      printErr:msg=>console.warn('[Fake-08]',msg)
    });
    Module._f08_init();
    return Module;
  })();
...
async function getRuntimeLua(){
  if(!luaPromise)luaPromise=fetch(new URL('celeste-preview.lua',RUNTIME_BASE),{cache:'no-store'}).then(r=>{
    if(!r.ok)throw new Error(`Could not load PICO-8 preview runtime (${r.status})`);
    return r.text();
  });
  return luaPromise;
}

async function getAtlasIndices(){
  if(!atlasPromise)atlasPromise=(async()=>{
    const img=new Image();img.decoding='async';
    await new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=()=>reject(new Error('Could not load Celeste sprite atlas'));img.src=ATLAS_URL.href;});
    if(img.width!==128||img.height!==64)throw new Error(`Expected 128x64 Celeste atlas, got ${img.width}x${img.height}`);
    const c=document.createElement('canvas');c.width=128;c.height=64;
    const x=c.getContext('2d',{willReadFrequently:true});x.imageSmoothingEnabled=false;x.drawImage(img,0,0);
    return atlasRgbaToPicoIndices(x.getImageData(0,0,128,64).data);
  })();
  return atlasPromise;
}
...
function loadCart(Module,bytes){
  ...
  try{
    ...
    if(result!==0)throw new Error(lastFake08Error(Module)||`Fake-08 rejected the generated cartridge (code ${result})`);
...
export function createPico8Preview({canvas,status,dialog}){
  ...
  let held=0,pressed=0,grab=false,cartBytes=null;
  ...
  function onKeyDown(e){
    ...
    if(e.code==='KeyR'){
      e.preventDefault();
      if(cartBytes){loadCart(Module,cartBytes);held=0;pressed=0;grab=false;accum=0;last=performance.now();setStatus('Restarted · real Fake-08 PICO-8 runtime');}
      return;
    }
  ...
  function onKeyUp(e){
    ...
    const bit=KEY_BITS[e.code];
    if(bit){held&=~bit;if(active())e.preventDefault();}
  }
  ...
  function loop(now){
    if(!running)return;
    accum+=Math.min(150,now-last);last=now;
    let steps=0;
    while(accum>=frameMs&&steps<6){
      Module._f08_set_inputs(pressed,held,0,0,grab?1:0);
      pressed=0;
  ...
  async function start(level){
    ...
    setStatus('Building level cartridge and loading the real PICO-8 runtime…');
    const [mod,atlas,runtimeLua]=await Promise.all([getFake08(),getAtlasIndices(),getRuntimeLua()]);
    Module=mod;
    const cart=buildPico8Cart(level,{atlasIndices:atlas,runtimeLua});
    cartBytes=new TextEncoder().encode(cart);
    loadCart(Module,cartBytes);
    fbPtr=Module._f08_get_framebuffer_ptr();
    if(!fbPtr)throw new Error('Fake-08 did not expose a PICO-8 framebuffer');
    if(!palettePtr)palettePtr=Module._malloc(64);
    const fps=Module._f08_get_target_fps?.()||30;
    frameMs=1000/(fps>0?fps:30);
    held=0;pressed=0;grab=false;accum=0;last=performance.now();running=true;
    setStatus(`Real PICO-8 cartridge running in Fake-08 @ ${Math.round(1000/frameMs)} Hz · Z jump · X dash · C grab · R restart`);
    Module._f08_set_inputs(0,0,0,0,0);Module._f08_step_frame();renderFrame();
    raf=requestAnimationFrame(loop);
  }

  function stop(closeDialog=true){
    running=false;cancelAnimationFrame(raf);raf=0;held=0;pressed=0;grab=false;
