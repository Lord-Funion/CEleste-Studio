...
export const constants = Object.freeze({
  MAGIC: 'CELV', VERSION, KIND_LEVEL, KIND_PACK, HEADER_SIZE, MAX_APPVAR_PAYLOAD,
});
...
function decodeRoom(reader, index) {
  ...
  const flags = reader.u8(); reader.u8();
  ...
  return { id, width, height, spawnX, spawnY, exitX, exitY, flags, tiles, entities };
}
...
export function decodePayload(input) {
  ...
  const version = r.u8(); if (version !== VERSION) throw new Error(`Unsupported CEleste level format version ${version}`);
  ...
  if (kind === KIND_LEVEL) {
    const rooms = [];
    for (let i = 0; i < itemCount; i++) rooms.push(decodeRoom(r, i));
...
export function validateLevel(level) {
  ...
  (level.rooms ?? []).forEach((room, i) => {
    ...
    if (room.width !== 16 || room.height !== 16) warnings.push(`${label}: the current calculator runtime is optimized for 16x16 rooms.`);
    ...
    if (room.exitX < 0 || room.exitX >= room.width || room.exitY < 0 || room.exitY >= room.height) errors.push(`${label}: exit is outside the room.`);
    const seen = new Set();
    for (const entity of room.entities ?? []) {
      if (entity.x >= room.width || entity.y >= room.height) errors.push(`${label}: entity ${entity.type} is outside the room.`);
      const key = `${entity.x},${entity.y}`; if (seen.has(key)) warnings.push(`${label}: multiple entities occupy ${key}.`); seen.add(key);
    }
  });
